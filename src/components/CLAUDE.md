...
| File | Responsibility |
|------|---------------|
| `CookingQueue.jsx` | Main dish-library panel: lists saved dishes with category filtering, queue selection, category management, shopping-list merge modal, recipe-detail modal, and export/import buttons (导出备份 / 导入备份). Hosts `useImportBackup` and mounts `ImportConflictModal`. |
...
| `Header.jsx` | App header with navigation and sync-state indicator. |
